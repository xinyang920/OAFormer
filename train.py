    ...
    def save(self):
        ...
        with torch.no_grad():
            for image, mask, shape, name in self.loader:
                ...
                for i, once in enumerate(ax1):
                    once = torch.unsqueeze(once, dim=0)
                    once = F.interpolate(once, size=(shape[0][i].item(),shape[1][i].item()), mode='bilinear', align_corners=True)
                    once = torch.sigmoid(once).cpu().numpy() * 255
                    cv2.imwrite(head+'/'+name[i]+'_ax1.png', once[0][0])
                for i, once in enumerate(ax2):
                    once = torch.unsqueeze(once, dim=0)
                    once = F.interpolate(once, size=(shape[0][i].item(),shape[1][i].item()), mode='bilinear', align_corners=True)
                    once = torch.sigmoid(once).cpu().numpy() * 255
                    cv2.imwrite(head+'/'+name[i]+'_ax2.png', once[0][0])
                for i, once in enumerate(ax3):
                    once = torch.unsqueeze(once, dim=0)
                    once = F.interpolate(once, size=(shape[0][i].item(),shape[1][i].item()), mode='bilinear', align_corners=True)
                    once = torch.sigmoid(once).cpu().numpy() * 255
                    cv2.imwrite(head+'/'+name[i]+'_ax3.png', once[0][0])
                # for i, once in enumerate(ax3):
                #     once = torch.unsqueeze(once, dim=0)
                #     once = F.interpolate(once, size=(shape[0][i].item(),shape[1][i].item()), mode='bilinear', align_corners=True)
                #     once = torch.sigmoid(once).cpu().numpy() * 255
                #     cv2.imwrite(head+'/'+name[i]+'_high.png', once[0][0])
